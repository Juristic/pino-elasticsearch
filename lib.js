...
function pinoElasticSearch(opts = {}) {
   if (opts['flush-bytes']) {
      process.emitWarning(
         'The "flush-bytes" option has been deprecated, use "flushBytes" instead'
      );
   }

   if (opts['flush-interval']) {
      process.emitWarning(
         'The "flush-interval" option has been deprecated, use "flushInterval" instead'
      );
   }

   if (opts['es-version']) {
      process.emitWarning(
         'The "es-version" option has been deprecated, use "esVersion" instead'
      );
   }

   if (opts['bulk-size']) {
      process.emitWarning(
         'The "bulk-size" option has been removed, use "flushBytes" instead'
      );
   ...
   class CustomTransport extends Transport {
      async request(params, options) {
         params.path = `${url.pathname}${params.path}`;
         return super.request(params, options);
      }
   }

   clientOpts.Transport = CustomTransport;
